...
export function UseCases() {
  ...
  return (
    <section ref={sectionRef} id="use-cases" className="py-16 sm:py-24 border-b dark:border-white/5 border-neutral-200 relative">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center mb-10">
          <div className="inline-flex items-center gap-2 p-1.5 rounded-xl dark:bg-neutral-900 bg-neutral-100 border dark:border-white/10 border-neutral-200 backdrop-blur-md">
            <button
              onClick={() => setActiveTab('designers')}
              className={`px-4 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition-all flex items-center gap-2 ${
                activeTab === 'designers'
                  ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/25'
                  : 'dark:text-neutral-400 text-neutral-600 hover:text-foreground'
              }`}
            >
        ...
        <div ref={cardRef} className="mx-auto max-w-4xl rounded-2xl border dark:border-white/10 border-neutral-200 dark:bg-neutral-900/60 bg-white p-6 sm:p-10 backdrop-blur-2xl shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-80 h-80 bg-amber-500/10 blur-[100px] pointer-events-none rounded-full" />

          <div className="relative z-10 space-y-8">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b dark:border-white/10 border-neutral-200 pb-6">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500">
                  <IconComponent className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-2xl font-bold dark:text-white text-neutral-900">{currentCase.title}</h3>
                  <p className="text-xs sm:text-sm dark:text-neutral-400 text-neutral-600">{currentCase.description}</p>
                </div>
              </div>
              <span className="text-xs font-mono uppercase bg-amber-500/10 text-amber-600 dark:text-amber-400 px-3 py-1 rounded-full border border-amber-500/20 font-semibold">
                {currentCase.badge}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {currentCase.benefits.map((benefit, i) => (
                <div key={i} className="flex items-start gap-3 dark:bg-neutral-950/40 bg-neutral-50 p-4 rounded-xl border dark:border-white/5 border-neutral-200">
                  <CheckCircle2 className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
                  <span className="text-xs sm:text-sm dark:text-neutral-300 text-neutral-700 leading-relaxed">{benefit}</span>
                </div>
              ))}
            </div>

            {/* Testimonial Quote */}
            <div className="p-4 sm:p-5 rounded-xl dark:bg-neutral-950/60 bg-amber-500/5 border border-amber-500/20 text-xs sm:text-sm dark:text-amber-300/90 text-amber-800 italic font-light">
              {currentCase.quote}
            </div>
          </div>
        </div>
